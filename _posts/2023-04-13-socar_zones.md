...
프로젝트의 상세한 진행경과은 [노션](https://modulabs.notion.site/e9651dcbc43c4d0e95954ef1963426f1)에 기록되어 있습니다.
...
각각의 단계에 대한 설명은 다음과 같습니다.
 ...
 - 수요예측 : 각 군들에 대하여 가장 수요가 증가할 것으로 예상되는 군집(지역)을 선정한다.
 - 신규존 제안 : 선정한 지역에 대하여 신규 쏘카존의 위치를 제안한다.

<div class="notice--danger">
이 프로젝트에서 <code>이용 수</code>와 '수요'는 같은 말입니다.
쏘카 서비스가 몇 회 이용되었는지를 나타내는 수치입니다.
feature를 나타낼 때에는 <code>이용 수</code>라는 단어를, 일반명사의 의미에서는 '수요'라는 단어를 사용하겠습니다.
이것은 문맥에 따라 지역별 수요가 될 수도 있고, 날짜별 수요가 될 수도 있으며, 군집별 수요가 될 수도 있습니다.
</div>
...
**하지만, 만약 42개 지역을 군집화하고, 16개 지역에 대한 수요들을 바탕으로 군집별 수요 증감을 예측한다면, 수요 정보가 없는 지역에 대해서도 수요의 증감을 간접적으로 예상할 수 있으며, 필요하다면 해당지역에 대해서도 쏘카존 위치를 제안할 수 있을 것입니다.**
...
- `인구`는 직접적으로 데이터를 얻을 수 있었으나, `2030인구`(20대, 30대 인구)는 직접 데이터를 얻을 수는 없었습니다.
하지만 각 광역(자치구를 가지고 있는 대도시)의 나이대별 인구비율 데이터를 얻을 수 있었고, 이를 `인구` 데이터와 결합하여 `2030인구`를 간접적으로 구했습니다.
...
상관분석을 통해 얻은 11개의 feature들의 목록은 다음과 같습니다.
