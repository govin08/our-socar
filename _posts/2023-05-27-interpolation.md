...
univariate interpolation이라는 말에서 변수가 한 개(univariate, 일변수, 1D)라는 의미는 독립변수($x$)의 개수가 한 개라는 것입니다.
...
둘째로, 보간해야 할 column이 물리적인 의미를 가지고 있을 때 cubic spline이 가지는 의미는 더 커집니다.
