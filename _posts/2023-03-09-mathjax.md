...
확장자가 같이 표시된 것은 파일이며, 확장자가 없는 것은 폴더입니다.
모든 파일/폴더를 표시하지는 않았고, 한 번이라도 만져본 적이 있는 파일/폴더만 표시했습니다.
파일/폴더 오른쪽에 '%'로 주석을 달아보았습니다.

## 블로그 구조
```
govin08.github.io
├ _data                                         % 
  └ navigation.yml                              % 카테고리 지정
├ _includes                                     % 
    ├ head                                      % 
        └ custom.scss                           % use_math 옵션을 지정
    └ mathjax-support.html                      % mathjax 파일
├ _layouts                                      % 각 post의 layout이 정의되어 있는 폴더
├ _pages                                        %
    ├ 404.md                                    % 404 error가 났을 때 표시하는 페이지
    ├ about.md                                  % Introduce에 나타나는 페이지
    ├ category-acrchive.md                      % Category에 나타나는 페이지
    └ tag-archive.md                            % Tag에 나타나는 페이지
├ posts                                         % 
    ├ 2023-02-13-maupassant_1_la_parure.md      % 포스트 : 모파상 - 목걸이
    ├ 2023-02-13-maupassant_2_boule_de_seif.md  % 포스트 : 모파상 - 비곗덩어리
    ├ 2023-02-15-maupassant_3_une_vie.md        % 포스트 : 모파상 - 여자의 일생
    ├ 2023-02-25-maupassant_4_bel_ami.md        % 포스트 : 모파상 - 벨아미
    ├ 2023-03-01-diagonalization.md             % 포스트 : 행렬의 대각화
    └ 2023-03-09-mathjax.md                     % 포스트 : mathjax
├ _sass
    └ minimal-mistakes                          %
        └ _pages.scss                           % 페이지의 전체적인 레이아웃을 조정할 수 있음.
        └ _utilities.scss                       % e.g. 메인 페이지의 폭을 조정할 수 있음
        └ _variables.scss                       % minimal-mistakes 폴더 내의 파일들에 정의된 변수들이 정의되어 있음
├ .github                                       % git init 시 생성되는 폴더
├ assets                                        %
    └ images                                    % 블로그의 시스템적인 이미지 파일들이 들어있는 폴더
├ images                                        % 각 포스트에 첨부되는 이미지 파일들이 들어있는 폴더
├ config.yml                                    % (중요) 블로그의 전반적인 configuration을 설정하는 파일
└ readme.md                                     % github에 표시되는 그 readme 파일
```
`mathjax`를 설정하기 위해 바꾼 추가하거나 수정한 파일들은
...
이 파일은 지금 쓰고 있는 포스트를 작성하기 위해 만든 마크다운 파일입니다.
이 파일의 맨 앞에는 원래라면 
```
---
layout: single
...
```
와 같이 만들어줍니다.
이제, `mathjax`를 사용하여, LaTeX의 문법을 '제한적으로' 사용할 수 있습니다.
...
display 모드의 수식 입력시 달러 사인 두 개 `$$`를 수식의 양 옆에 쓰는 것은 먹히지만 ; (1), (3), 이것을 소괄호나 대괄호의 형태로 쓰이는 것은 먹히지 않습니다 ; (2), (4).
...
위의 문제들 중 (2), (4)는 (1), (3)과 대체할 수 있고, (5) 또한 마크다운 형식의 enumeration/itemization을 사용하면 되어서 크게 문제가 되지 않을 수 있습니다.
...
$$gH=Hg=H.$$
It is worthwhile to note that $gH$ form a group itself ; we denote this new group by $G/H$.
Let

$$G/H=\{gH:g\in G\}.$$

Then, it is indeed a group under the following operation
...
# 참고자료들
 ...
 - [Add LATEX support to your blog by Zixu Wang](https://quuxplusone.github.io/blog/2018/08/05/mathjax-in-jekyll/)
    - 가장 직접적으로 참고한 자료입니다.
    - 하지만 이것이 최선의 솔루션은 아니라고 판단됩니다.
 - [EP21. LaTeX 수식 문법을 지원하는 mathjax를 블로그에 적용하기 by 테디노트](https://youtu.be/3O08iA_BFbM)
    - 가장 쉬운 설명이지만, display math 모드의 수식 지원 및 `align` 환경이 지원되지 않습니다.
 - [github blog와 latex by euns](http://www.ktug.org/xe/index.php?mid=KTUG_open_board&search_keyword=mathjax&search_target=title_content&document_srl=248288)
    - 해당 작성자분이 KTUG에서 발표한 내용이니만큼 가장 완성도있는 글이라고 생각됩니다. 자료도 꽤 최신(2020)입니다.
    - 하지만, 제가 한 것처럼 `minimal-mistakes` 테마에 적용한 것이 아닌 `minima` 테마에 적용했기에, 똑같이 따라했을 때 오류가 납니다.
 - [MathJax v3 in Jekyll by Arthur O’Dwyer](https://quuxplusone.github.io/blog/2020/08/19/mathjax-v3-in-jekyll/)
    - `mathjax`의 버전에 따라 설정을 변경해야 함을 환기하고 있습니다.
    - 이 포스트의 내용 역시 잘 참고하고 적용할 수 있다면 아주 좋겠습니다만, 다 이해하지 못했습니다.
 - [mathjax 공식 홈페이지](https://www.mathjax.org/)
    - `mathjax`를 제대로 활용하려면 응당 정독해야할만한 공식 documentation입니다.
    - demo 또한 지원하고 있어서, 실제 `mathjax`를 적용했을 때 어떻게 웹상으로 렌더링되는지 보여주기도 합니다.
    이 demo로 테스트해보면 소괄호와 대괄호를 이용한 수식 입력도 적용된다는 것을 알 수 있습니다.

따라서, 아직은 수식 입력에 관한 세팅을 완벽하게 했다고 볼 수는 없습니다.
추후 보완할 사항들은 다음과 같습니다.

- 대괄호/소괄호를 이용한 수식 입력될 수 있도록 하면 더 좋다.
- `mathjax` v3이 잘 적용되고 있는지 확인해야 한다.
- `mathcal`, `mathbb`, `mathcal` 등이 아직 적용되는지 테스트해보지 않았다.